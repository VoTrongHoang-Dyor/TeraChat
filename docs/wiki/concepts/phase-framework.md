...
| **.tapp Web Marketplace** | Self-service: IT admin browses → downloads → deploys to region/department. All t-apps vetted by TeraChat. Simple setup instructions per t-app. Regional/departmental scoping built in. | TERA-ECO covers app signing, registry, kill-switch. Marketplace browsing/search/self-service not yet specified. |
...
| **.tapp Marketplace (Full)** | Third revenue stream: 30% publisher revenue share. Only valuable with 50+ enterprise customer base. Third-party publishers with full vetting pipeline. | TERA-ECO covers app signing, registry, kill-switch. Marketplace billing/search/review not yet specified. |
