...
| **TD-015** | `TERA-ECO` | **Federation Directory Lookup Specs Chưa Implement** <br> *Mô tả:* Federation Directory để lookup public keys giữa các tổ chức chưa có implementation cụ thể, gây khó khăn cho việc thiết lập kết nối federation. | 🟠 HIGH | Tồn đọng | Federation launch | Thêm DNS-like lookup service. TeraChat publish Ed25519 public key của Enterprise CA, verify và init mTLS. Tham chiếu: `Spec-Ecosystem-And-Trust-Chain.md` |
| **TD-016** | `TERA-CORE` | **Thermal Budget Architecture Khuyết Thiếu Spec** <br> *Mô tả:* E2EE Rust core, BLE mesh, WASM sandbox gây overheating nếu không có thermal budget management rõ ràng trên mobile devices (iOS/Android). | 🔴 CRITICAL | Tồn đọng | Edge mobile launch | Triển khai `ThermalStateMonitor` trong `Spec-Core-Cryptography-And-Mesh.md` với throttling protocol (.nominal, .fair, .serious, .critical) và `CoreSignal::ThermalThrottling`. |
...
| **TD-005** | ZK Memory Agent IPC Contract Chưa Được Đặc Tả | `v1.0.0` | Đã đóng: ZK Memory Agent nay giao tiếp qua Unix Domain Socket (UDS); Consolidation trigger đã được làm rõ tại TERA-ENCLAVE. |
